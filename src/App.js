...
function App() {
  ...
  const setBaseData = () => {
    axios
      .get("https://notion-next4-lfals.vercel.app/tickets")
      .then((result) => {
  ...
  const filterByYear = (clientYear, data) => {
    ...
    data.forEach((element) => {
      ...
      if (clientData === clientYear) {
        const fixHour = parseFloat(
          element.hours.replace("", 0).replace("h", "").replace(",", ".")
        );
  ...
  const getYear = (data) => {
    ...
    filteredDateArray.forEach((year) => {
      filteredYearOptions.push({ value: year, label: year });
    });
  ...
  const getMonths = (data) => {
    ...
    filteredMonthArray.forEach((month) => {
      filteredMonthsOptions.push({ value: month, label: month });
    });
  ...
  return (
    <>
      <div className="navbar sans">
        <a
          className="how"
          href="https://notion.next4.net.br/video/notion-listagem-tasks.html"
          target="_blank"
          rel="noreferrer"
        >
        ...
        <a
          href="https://www.notion.so/32973b3d397c4f1292da0f641908283a?v=26d0ae7fd930488db76ff7a56de7a5dc"
          target="_blank"
          rel="noreferrer"
          className="no-print"
        >
      ...
      <header className="sans">
        ...
        <h1 className="page-subtitle">
          {sumHour ? `Somatória das horas = ${sumHour}h` : ""}
        </h1>
      </header>
      ...
      <div className="page-body sans">
        <Table className="collection-content">
          ...
          <TableBody>{data.map((ticket, i) => objectRow(ticket, i))}</TableBody>
        </Table>
      </div>
    </>
  );
